...
def load_and_combine_npy_files(folder, feature_prefix, info_prefix):
    feature_files = sorted(
        [
            f
            for f in os.listdir(folder)
            if f.startswith(feature_prefix) and f.endswith(".npy")
        ]
    )
    info_files = sorted(
        [
            f
            for f in os.listdir(folder)
            if f.startswith(info_prefix) and f.endswith(".npy")
        ]
    )
    ...
    for feature_file, info_file in zip(feature_files, info_files):
        features = np.load(os.path.join(folder, feature_file))
        info = np.load(os.path.join(folder, info_file), allow_pickle=True)

        all_features.append(features)
        all_info.append(info)

    combined_features = np.vstack(all_features)
    combined_info = np.vstack(all_info)

    return combined_features, combined_info


# Load image features và thông tin từ file .npy
try:
    image_features, image_info = load_and_combine_npy_files(
        "result/", "keyframes_features", "keyframes_info"
    )
...
def get_neighbor_image_details(image_name, num_neighbors=5):
...
def translated_text(sid, data):
    ...
    top_indices = np.argsort(similarities[0])[::-1][:60]
    ...
    for i, index in enumerate(top_indices):
        ...
        for idx, neighbor_image_name in enumerate(neighbor_image_names[:5], start=1):
            img_path = os.path.join(
                image_folder, folder_name, neighbor_image_name + ".jpg"
            )
            ...
            result_json[f"near{i+1}.{idx}"] = {
                "keyframe": folder_name,
                "name": neighbor_name,
                "frameid": neighbor_frame_id,
            }
        ...
        for idx, neighbor_image_name in enumerate(neighbor_image_names[5:], start=6):
            img_path = os.path.join(
                image_folder, folder_name, neighbor_image_name + ".jpg"
            )
            neighbor_image_name_int = str(int(neighbor_image_name))
